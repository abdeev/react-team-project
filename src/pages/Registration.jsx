...
const Registration = () => { 
    ...
    const handleChange = ({ target: { name, value } }) => {
        switch (name) {
            case 'username': return setName(value);
    ...
    return (
        <form className="mx-auto my-9 flex flex-col p-7 rounded-xl border border-solid border-black bg-green-300" onSubmit={handleSubmit}>
            ...
            <input className="mb-6 text-2xl font-sm py-1 px-5 rounded-xl border border-black" id="name" type="text" name="username" onChange={handleChange} value={username} pattern="^[a-zA-Zа-яА-Я]+(([' -][a-zA-Zа-яА-Я ])?[a-zA-Zа-яА-Я]*)*$" placeholder="Enter name"
